...
interface outputObject {
   byId: {
      [id: string]: inputObject
   }
   allIds: string[]
}


const normalizeData = <T extends inputObject[]>(unnormalizedData: T): outputObject => {
   return ({
      byId: unnormalizedData.reduce((accumulator, value) => ({ ...accumulator, [value.id]: value }), {}),
